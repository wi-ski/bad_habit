...
module.exports = function (router) { //might refactor to use _id(mongo atm)
	...
	router.get("/nmc/entry/:attr/",function ( req,res ) { //should return the last 20 entries
		var searchObj = {};
		...
		NmcBlock.find().limit(20).exec(function (err,MongoDocument) { //<- NOT ideal, would like to use the block height to find some number of entries
			if(err){
				res.send.status(404).send(_badRequest("Error - name GET request",err));
			}
	...
	router.get("/nmc/entry/:attr/:val",function ( req,res ) {
		...
		NmcBlock.find(searchObj,function (err,MongoDocument) {
			if(err){
				res.send.status(404).send(_badRequest("Error - " + req.params.attr + "GET request for : " + req.params.val,err));
			}
			res.send(_goodRequest(MongoDocument));
		});
	});
	...
	router.post("/login",function ( req,res ) {
		...
		User.get(req.body,function(err,UserObj){
			...
			UserObj.authenticate(req.body.password,function(err,resp){
				if(err) return res.send(_badRequest("Something went wrong, this might be our fault"));
				if(!resp) return res.status(404).send(_badRequest("Something went wrong, check email and password"));
